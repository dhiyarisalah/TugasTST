...
def common_word_lyrics(name, year, target):
...
def get_all(name, year, target):
    user_result = {}
    first_dict = get_music_mood(name, year)
    user_result.update(song_recommendations=first_dict)
    second_dict = get_music_mood(name, year)['mood']
    ...
    if (target == 'mood'):
        ...
        return user_result
    if (target == 'lyrics'):
        third_dict = common_word_lyrics(name,year, "only_quotes")
        user_result.update(quotes=third_dict)
        forth_dict = common_word_lyrics(name,year, "all")['quotes_by_film']
        user_result.update(quotes_by_film=forth_dict)
        fifth_dict = common_word_lyrics(name,year, "all")['film_recommendations']
        user_result.update(film_recommendations=fifth_dict)
    return user_result